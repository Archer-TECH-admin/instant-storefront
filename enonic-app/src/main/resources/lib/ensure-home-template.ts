...
const SUPPORTS = 'portal:site';
const PAGE_DESCRIPTOR = `${APP}:home`;
...
export function ensureHomeTemplate(): void {
    log.info('[ensure-home-template] checking…');
    ...
    if (onDraft.total > 0) {
        ...
        log.info(`[ensure-home-template] found on draft, re-applying components: ${hit._path}`);
        ...
        if (folderResult.total === 0) {
            log.warning('[ensure-home-template] no template folder found, skipping');
            return;
        }
        ...
        if (!created) {
            log.error('[ensure-home-template] contentLib.create returned null');
            return;
        }
        nodePath = '/content' + created._path;
        log.info(`[ensure-home-template] created: ${created._path}`);
    ...
    log.info(`[ensure-home-template] pushed: success=${ok} failed=${ko}`);
}
